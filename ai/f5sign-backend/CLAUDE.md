...
- PostgreSQL 18 (§2.3) — corrected 2026-09-15; this line read `PostgreSQL 16` until then
...
⛔ **FIRST: are you in a git worktree? Then every target in the table below validates SOMEONE ELSE'S CODE, and reports green for it.** `f5sign-infra`'s `docker-compose.override.yml` bind-mounts **`../f5sign-backend`** — the *main* checkout, hardcoded — into the container. A worktree is never mounted, so `make test`, `make phpstan`, `make lint` and `make composer cmd=…` all run against whatever branch the main checkout happens to be sitting on. Your edits are not in the container, and **nothing warns you**: the run succeeds, the counts look plausible, and the answer is about another branch. ⚠ **Verify that path with `git -C ../f5sign-infra diff` and not only by reading it.** On 2026-08-25 two sessions independently concluded the bind was `../f5sign-backend-develop` — one by `grep` on the file, one by `docker inspect` — and **both were reading an uncommitted local edit**: someone had repointed the four lines in the working tree and never committed them. The working tree and the running container agree with each other by construction, so neither can tell you what the repository says, and one of those sessions rewrote this very paragraph to document that local state as the design before the other caught it. The committed value is `../f5sign-backend`; if what you observe disagrees, the divergence is on your machine and is the thing to fix, not this sentence.
