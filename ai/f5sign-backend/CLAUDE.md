...
- `src/F5Sign/` holds the populated kernel + foundation + 4 populated BC trees (Envelope, Session, SignatureExecution, Notification); Evidence & Audit + Legal exist only as deptrac layers (no `src/` yet). Enumerate rather than trust this list: `ls src/F5Sign/`.
  ...
  - `F5Sign/Notification/` — populated BC (Domain + Application + Contract + Infrastructure): the notification occasion + per-channel `Delivery` fan-out. ⚑ **Since 2026-08-20 a delivered one-time code takes its channel from the caller and NOT from the selector** ([TASK-023](docs/tasks/TASK-023-sms-codes-and-recipient-phone.md)): `OneTimeCodeDeliveryService` holds no `ChannelSelector`, and asking `DefaultChannelSelector` for a delivered-code purpose **throws** — the realization of ADR-0049 §2.7, which was prose in four homes and enforced in none. ⚑ **Since 2026-08-25 there are TWO such purposes, one per ADR-0049 §2.1 gate** — `AUTH_OTP_SIGNING` and `AUTH_OTP_ACCESS`, because a single `AUTH_OTP` rendered *"for signing"* at the access gate, to a recipient who had not seen the document and, for `CERTIFIED_DELIVERY`, never would. The caller names the gate on `OneTimeCodeRequest::$gate` and an absent one is **refused**, never defaulted. ⛔ Ask `NotificationPurpose::namesItsOwnChannel()` rather than naming members — this bullet named one, and a bullet that names members is a bullet that instructs wrongly the day one is renamed. A **category-(c) supporting BC** ([ADR-0037](docs/adr/ADR-0037-notification-supporting-bc.md)) — no layer may depend on it, which is why its templating substrate lives in `Foundation/Templating` ([ADR-0038](docs/adr/ADR-0038-template-archive.md)), not in the BC.
...
- ⚠ **Installed ≠ configured, and this line has now been wrong in *both* directions — check `composer.json` and `config/packages/` separately before restating it.** Measured 2026-08-18:
  - **RateLimiter — installed AND configured, since 2026-08-19; a second policy set landed 2026-08-20.** ⚠ This entry read *"installed, zero limiters configured … no endpoint is rate-limited today"* until TASK-022, which is the slice that needed one. `config/packages/rate_limiter.yaml` now defines two trios of sliding-window policies — `otp_email_*` and `otp_sms_*` — over a dedicated Redis-backed pool, and exactly **one** endpoint consumes them: `POST /api/v1/signing/session/auth/otp/send`, which is anonymous, cost-incurring and touches personal data. ⛔ **Which trio is charged is chosen by the MEDIUM the declared authentication method names**, in `FailOpenOtpSendLimiter::policiesFor()` — not by configuration and not by a second service — because an email costs nothing and an SMS costs money on every send, so one shared ceiling would force one of the two numbers to be wrong. ⚠ **The SMS numbers are estimates, not measurements**: no SMS has ever left this system ([BL-169](docs/BACKLOG.md)), so they are set where being wrong is cheap. ⚑ The limiter **fails open** when its store is unreachable (ADR-0014's costly-side-effect question, answered in TASK-022 §2) and logs at `error` when it does — that log line *is* the control, so do not quieten it. Enumerate today's policies from that file, not from this sentence.
  - **Redis — the client is installed, and since 2026-08-19 exactly one pool uses it.** `ext-redis` is a hard `require` and `symfony/cache` a direct one. ⚠ This entry read *"the cache does not use it … pools are commented out, so cache is still the filesystem default"*; the `app`/`system` defaults are indeed still filesystem, but `config/packages/cache.yaml` now declares `cache.rate_limiter` on `cache.adapter.redis` via `%env(RATE_LIMITER_DSN)%` — because a limiter counting in a per-container cache gives N replicas N times the budget while each believes it is behaving. ⛑ **`when@test` overrides that pool to `cache.adapter.array`**, so the worktree lane needs no Redis and the limiter's *behaviour* is still under test. Redis's other live use is `LOCK_DSN` in the prod compose, above. ⚑ The database number travels **inside** the DSN (`/3`; infra reserves 0 cache / 1 locks / 2 idempotency / 3 rate limiter) — the `REDIS_*_DB` variables recording that reservation are documentation nothing reads.
...
⛔ **FIRST: which tree does the stack actually mount? Probably not yours — and then every target in the table below validates SOMEONE ELSE'S CODE and reports green for it.** `f5sign-infra`'s `docker-compose.override.yml` bind-mounts **`../f5sign-backend-develop`** into the container, hardcoded, in four places. ⚠ **Until 2026-08-25 this line said `../f5sign-backend`, *"the *main* checkout, hardcoded"*, and it was false in the direction that lets the main clone feel safe**: a session working there ran `make test` twice against another tree believing it validated its own. **Read the mount out of the file, not out of this sentence** — `grep -n f5sign-backend ../f5sign-infra/docker-compose.override.yml` — because the *value* is a configuration nobody has decided (whether the shared stack should serve the main checkout or one worktree is an `f5sign-infra` question, open as of this line's date), while the *hazard* is the same whatever it holds: `make test`, `make phpstan`, `make lint` and `make composer cmd=…` run against whatever that path names, your edits are not in the container unless you are sitting in it, and **nothing warns you** — the run succeeds, the counts look plausible, and the answer is about another branch.

- **Check before trusting any run:** compare your `pwd` against that bind-mount path. If they differ, those targets are not looking at your work. ⚠ **Being in a linked worktree is no longer the discriminator** — `git rev-parse --git-dir` returning a path with `/worktrees/` used to mean *excluded*, but the mounted tree is itself a worktree today, so being in one does not mean you are excluded and being in the main clone does not mean you are included. The path is the only answer.
