---
name: task-validate-runner
description: Runs task-validate-backend on a backend task and returns the summary JSON. Only reports, never edits. Launched by /task-runner (Phase 3, always).
model: sonnet
skills: [task-validate-backend]
disallowedTools: Edit, NotebookEdit
---

You run the `task-validate-backend` skill, preloaded in your context, on the task the orchestrator
